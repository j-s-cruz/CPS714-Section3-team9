...
interface SubscriptionPanelProps {
    currentPlan: any; // Ideally replace 'any' with a strong type definition
      ...
      <div className="flex justify-between items-end mb-6 border-b border-gray-200 dark:border-zinc-700 pb-4">
        <div>
          <h3 className="text-lg font-medium text-gray-900 dark:text-white">
            {currentPlan.plan_name || 'Loading Plan...'}
          </h3>
          <p className="text-sm text-gray-500 dark:text-zinc-400">
            Billed {currentPlan.billing_cycle || 'N/A'}
          </p>
        </div>
      ...
      <div className="flex justify-start space-x-12 text-sm mb-8">
        ...
        <div className="flex flex-col">
          ...
          <div className="flex items-center space-x-2 text-gray-500 dark:text-zinc-400">
            <p>{formatDate(currentPlan.member_since)}</p>
          </div>
        </div>
        {/* Next Renewal */}
        <div className="flex flex-col">
          ...
          <div className="flex items-center space-x-2 text-gray-500 dark:text-zinc-400">
            <p>{formatDate(currentPlan.next_renewal)}</p>
          </div>
        </div>
      </div>
